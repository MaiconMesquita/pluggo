...
class HomeProvider1 extends CustomChangeNotifier {
      ...
      try {
        ...
        if(response?.statusCode == 201) {
          if(entity == "driver") {
            ...
            appRouter.replaceAll([const MainHostRoute()]);
  ...
  Future<void> _registerUser(String? email) async {
    try {
      final response = await dinamicRepository.postRequest(
          url: ApiEndpoints.urlRegisterAccount,
          body: {
            "username": userProvider.name,
            "email": email,
            "onesignalId": Device.oneSignalUserID
          },
          popups: {},
          authorization: ''
      );
      ...
      userProvider.setId(login.userId);
      print('[Register] Novo usuário criado com ID: ${login.userId}');
    } catch (e) {
      print('[Register] Falha ao criar novo usuário: $e');
      rethrow;
    }
  }
