...
// use App\Domain\Entity\ValueObject\PersonalDocumentNameType;
use App\Domain\Entity\ValueObject\UserType;
use App\Domain\Exception\DeviceNotFoundException;
use App\Domain\RepositoryContract\{UserRepositoryContract, FileRepositoryContract};
...
class SignupProgress
{
    ...
    {        
        $user = $this->userRepository->findOneBy(["deviceId" => $input->deviceId]);
        ...
        // $selfiePhoto = $this->fileRepository->findMostRecentByUserIdAndType(userId: $user->getId(), personalDocumentNameType: new PersonalDocumentNameType("selfiePhoto"), status: true);
        // if (!$selfiePhoto) $missingFields[] = 'selfiePhoto';

        // if (empty($missingFields) && $step == 4) $step = 5; // Todos os campos da etapa 4 estão preenchidos        

        // Etapa 5: Verifica documentFront
        // $documentFront = $this->fileRepository->findMostRecentByUserIdAndType(userId: $user->getId(), personalDocumentNameType: new PersonalDocumentNameType("documentFront"), status: true);
        // if (!$documentFront) $missingFields[] = 'documentFront';

        // if (empty($missingFields) && $step == 5) $step = 6; // Todos os campos da etapa 5 estão preenchidos        

        // Etapa 6: Verifica documentBack
        // $documentBack = $this->fileRepository->findMostRecentByUserIdAndType(userId: $user->getId(), personalDocumentNameType: new PersonalDocumentNameType("documentBack"), status: true);
        // if (!$documentBack) $missingFields[] = 'documentBack';
        ...
        if (empty($missingFields) && $step == 5) $step = 6; // Todos os campos da etapa 3 estão preenchidos             
        ...
        if (empty($missingFields) && $step == 6) $step = 7; // Todos os campos da etapa 7 estão preenchidos        
